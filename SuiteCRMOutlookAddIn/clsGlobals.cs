...
namespace SuiteCRMOutlookAddIn
{
    ...
    public static class clsGlobals
    {
        ...
        public static string GetSMTPEmailAddress(MailItem mailItem)
        {
            ...
            if (((str2 = AddinModule.CurrentInstance.OutlookApp.ActiveExplorer().CurrentFolder.Name) != null) && (str2 == "Sent Items"))
            {
        ...
        public static string GetEmailAddressForExchangeServer(string emailName)
        {
            try
            {
                MailItem item = (MailItem)AddinModule.CurrentInstance.OutlookApp.ActiveExplorer().Application.CreateItem(OlItemType.olMailItem);
        ...
        public static string GetSenderAddress(MailItem mail, string type)
        {
            if (type == "SendArchive")
            {
                if (AddinModule.CurrentInstance.OutlookApp.Session.CurrentUser.AddressEntry.Type == "EX")
                {
                    return GetEmailAddressForExchangeServer(AddinModule.CurrentInstance.OutlookApp.Session.CurrentUser.AddressEntry.Name);
                }
                return AddinModule.CurrentInstance.OutlookApp.Session.CurrentUser.AddressEntry.Address;
            }
