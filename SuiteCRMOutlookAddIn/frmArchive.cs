...
namespace SuiteCRMOutlookAddIn
{
    public partial class frmArchive : Form
    {
        ...
        private void frmArchive_Load(object sender, EventArgs e)
        {
            ...
            if (AddinModule.CurrentInstance.settings.show_custom_modules)
            {
            ...
            foreach (MailItem item2 in AddinModule.CurrentInstance.OutlookApp.ActiveExplorer().Selection)
            {
                ...
                if (settings.AttachmentsChecked == true)
                {
                    try
                    {
                        if (itemFromID.Attachments.Count > 0)
                        {
                            ...
                            {
                                if (!clsSuiteCRMHelper.UploadAttahcment(new SuiteCRMClient.clsEmailAttachments { DisplayName = attachment.DisplayName, FileContentInBase64String = AddinModule.Base64Encode(attachment, itemFromID) }, str))
                                {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                base.Enabled = false;
                this.Cursor = Cursors.WaitCursor;
                foreach (object obj2 in AddinModule.CurrentInstance.OutlookApp.ActiveExplorer().Selection)
                {
                    ...
                    if (this.tsResults.Nodes.Count > 0)
                    {
                        ...
                        if ((this.type == "SendArchive") && (o.Recipients.Count > 0))
                        {
                            o.Send();
                        }
                ...
                if (!flag)
                {
                    MessageBox.Show(AddinModule.CurrentInstance.OutlookApp.ActiveExplorer().Selection.Count.ToString() + " Email has been successfully archived", "Success");
                    base.Close();
                }
            }
            catch (System.Exception exception)
            {
                MessageBox.Show("There was an error while archiving, please see the log for more information", "Error");
                base.Enabled = true;
                this.Cursor = Cursors.Default;
            }
        }
